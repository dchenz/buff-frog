...
public class EndScreenController {
	@FXML private StackPane screenPane;
	...
	public void trigger() {
		...
		move.setOnFinished(buttons -> { revealButtons(); });
	...
	private ImageView createView(String filename, double x, double y) {
		ImageView view = new ImageView(
			new Image(getClass().getResourceAsStream(filename), 600, 450, true, true));
	...
	private Button createExitButton(String message) {
		...
		menuButton.setOnAction(menu -> {
			...
			fadeScreen(fadeCover, 2.5, false, exit -> { exitToMenu(); });
		});
	...
	private void fadeScreen(
		Node object, double duration, boolean fadeOut, EventHandler<ActionEvent> ev) {
		Timeline fadeEffect = new Timeline(new KeyFrame(Duration.millis(duration * 10),
			fade -> { object.setOpacity(fadeCover.getOpacity() + 0.01 * (fadeOut ? -1 : 1)); }));
	...
	private void fadeText(Text displayText) {
		Timeline fadeTextEffect = new Timeline(new KeyFrame(Duration.millis(25),
			fade -> { displayText.setOpacity(displayText.getOpacity() + 0.01); }));
		fadeTextEffect.setCycleCount(100);
		fadeTextEffect.play();
	}

	private void fadeButton(Button menuButton) {
		Timeline fadeButtonEffect = new Timeline(new KeyFrame(Duration.millis(25),
			fade -> { menuButton.setOpacity(menuButton.getOpacity() + 0.01); }));
