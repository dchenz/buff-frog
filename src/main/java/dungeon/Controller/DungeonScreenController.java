...
public class DungeonScreenController {
	...
	@FXML
	private AnchorPane mapScore;
	
	@FXML
	private Text mapMoves;
	...
	@FXML
	private AnchorPane keySlot;
	
	@FXML
	private AnchorPane swordSlot;
	
	@FXML
	private AnchorPane potionSlot;
	
	@FXML
	private ImageView openObjectivesButton;
	...
	private int buildObjectivePane(AnchorPane taskPane, CompletableObjective task, int indent, int n) {
	...
	private AnchorPane createObjectivePreview() {
		...
		int nAddedPanes = 1;
		nAddedPanes += createObjectivePreviewPane(objectiveScreen, ObjectiveTypes.TREASURE, nAddedPanes);
		nAddedPanes += createObjectivePreviewPane(objectiveScreen, ObjectiveTypes.ENEMY, nAddedPanes);
		nAddedPanes += createObjectivePreviewPane(objectiveScreen, ObjectiveTypes.FLOOR_SWITCH, nAddedPanes);
		nAddedPanes += createObjectivePreviewPane(objectiveScreen, ObjectiveTypes.EXIT, nAddedPanes);
	...
	private int createObjectivePreviewPane(AnchorPane objective, ObjectiveTypes type, int n) {
		...
		if (nRequired != 0) {
			...
			pane.setLayoutX(n * ((600.0 - 100 * nObjectives) / (nObjectives + 1)) + (n - 1) * iconWidth);
			pane.setLayoutY(200);
	...
	private AnchorPane createGameScreenOverlay(String message, Button okButton, boolean darkTheme) {
		...
		pane.setStyle(String.format("-fx-background-color: rgba(%s, 0.5);",
				darkTheme ? "0, 0, 0" : "255, 255, 255"));
		...
		nextLevelButton.setOnAction(next -> {
			if (customLevelPlaying) {
				...
				if (nextDungeon < dungeons.size()) {
					...
					Timeline fadeOut = new Timeline(new KeyFrame(Duration.millis(25), fade -> {
						fadeCover.setOpacity(fadeCover.getOpacity() + 0.01);
					}));
	...
	private void levelFailed() {
		...
		Timeline fadeIn = new Timeline(new KeyFrame(Duration.millis(10), fade -> {
			screen.setOpacity(screen.getOpacity() + 0.01);
		}));
