...
public class SandboxScreenController {
	...
	@FXML
	private ImageView rowUpButton;
	
	@FXML
	private ImageView rowDownButton;
	
	@FXML
	private ImageView colUpButton;
	
	@FXML
	private ImageView colDownButton;
	
	// counters
	
	@FXML
	private Text rowCounter;
	
	@FXML
	private Text colCounter;
		...
		if (dungeonEntitiesOK()) {
			...
			for (String[] entity : entities) {
				...
				if (entity[3] != null) {
					...
					writer.write(String.format("{\"type\":\"%s\",\"x\":%s,\"y\":%s}", 
		...
		pane.setOnMouseClicked(highlight -> {
			if (selectedTile != null) {
				...
				if (pane == selectedTile) {
					...
					mapGrid.getChildren().forEach(tile -> {
						tile.setEffect(null);
					});
					selectedTile = null;
					selectTile(pane);
