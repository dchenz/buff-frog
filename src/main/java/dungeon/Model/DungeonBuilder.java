...
public abstract class DungeonBuilder {
    ...
    private Map loadDungeon() {
		...
		ObjectiveBuilder taskBuilder = new ObjectiveBuilder(
			countEntityType(entities, "treasure"), 
			countEntityType(entities, "switch"),
			countEntityType(entities, "enemy"),
    ...
    private void loadEntity(Map dungeon, JSONObject json) {
        ...
        case "player":
        	...
        	player.getStateProperty().addListener(new ChangeListener<Boolean>() {
        		...
        		public void changed(ObservableValue<? extends Boolean> observable,
        			...
        			} else assert false; // Assert: cannot die yet still be alive
        		}
        	});
        ...
        case "enemy":
        	...
        	enemy.getFollowingProperty().bind(itemImages.get(ItemTypes.POTION).visibleProperty().not());
        ...
        case "portal":
        	...
        	dungeon.add(onLoad(getConnectedPortal(json.getInt("id"), x, y)), x, y); // Add to Map
        	break;
        case "key":
    ...
    private Portal getConnectedPortal(int id, int x, int y) {
    	...
    	} else for (int i = 0; i < entities.length(); i++) {
    ...
    private void bindScoreCounter(Inventory items) {
    	...
    	// When the score counter property in Inventory changes, convert it to a string and update Text
    	scoreCounter.textProperty().bindBidirectional(items.getPointsProperty(), new NumberStringConverter());
    }
