...
public class EnemyMover {
	...
	private boolean moveTowardsPlayer(Enemy enemy) {
		...
		List<Coordinate> path =
			buildGraph(enemy.getPosition(), new Coordinate(playerX.get(), playerY.get()));
	...
	private boolean moveAwayFromPlayer(Enemy enemy) {
		...
		do {
			...
			!moved && attempts < 4); // while has not moved and attempts < 4 (increment tile key)
	...
	private List<Coordinate> buildGraph(Coordinate enemyPosition, Coordinate playerCoordinate) {
		...
		// get shortest path
		GraphPath<Coordinate, DefaultEdge> shortest =
			DijkstraShortestPath.findPathBetween(graph, enemyPosition, playerCoordinate);
	...
	private void createEdge(Coordinate src, Coordinate dest) {
		...
		if (portals.containsKey(dest)) {
			...
			dest =
				new Coordinate(portal.getDestinationXPosition(), portal.getDestinationYPosition());
		}
